...
private void Start(){Player=FindObjectOfType<PlayerControllerWMW2D>().gameObject;LastCheckPoint=0;CurrentItemsRespawnTime=ItemsRespawnTime;CurrentRespawnTime=NormalRespawnTime;EnemiesInPool();ItemsInPool();}

private void Update(){if(!GameManager._SharedInstanceGameManager.Cinematica){CheckPointActualization();
if(Player.GetComponent<PlayerControllerWMW2D>().Iddle==true){ActivationCrono-=Time.deltaTime;}
if(ActivationCrono<=0){RandomEnemiesRespawnCronometre();RandomItemsRespawnCronometre();}
if(ActivationCrono<=-30){EnemyHorde=true;}}else{}}
}
